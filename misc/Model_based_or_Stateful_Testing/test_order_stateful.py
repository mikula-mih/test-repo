...
# `hypothesis`
# going to test a whole range of automatically generated inputs according to
# some constraints and also test that exact sequence of actions
# >>> pytest
# let `hypothesis` choose which method to call next
# start by listing all the possible actions
# to all these different actions add sanity checks that validate that some
# property holds true
#
# the "actions" are called "rules"
# the "sanity checks" are called "invariance"
# together form a state machine
#
# also define "preconditions" that hold before the rule can actually be applied
