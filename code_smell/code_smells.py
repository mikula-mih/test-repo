...
def main():
    ...
    registry.add_vehicle_model_info(VehicleModelInfo("Tesla", "Model 3", 5000))
    registry.add_vehicle_model_info(VehicleModelInfo("Volkswagen", "ID3", 3500))
    registry.add_vehicle_model_info(VehicleModelInfo("BMW", "520e", 6000, FuelType.PETROL))
    registry.add_vehicle_model_info(VehicleModelInfo("Tesla", "Model Y", 5500))

    # verify that the registry is online
    print(f"Registry status: {registry.online_status()}")

    # registry a new vehicle
